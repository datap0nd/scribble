...
namespace Scribble.Office
{
    ...
    public static class SamsungAuthoringPolicy
    {
        ...
        public const string Instructions =
            ...
            "Every displayed number must be supported by the resolved source_spans, including numbers in titles, subtitles, takeaways, chart categories and series, tables, footnotes, citations and row identifiers. For an existing presentation, inspect_slide.citation_text is the decoded native text to quote verbatim in claims and calculations; the structured content JSON is for geometry and editing, not for reconstructing a source sentence. Cite a summary passage for an aggregate that it states directly; raw detail rows do not verify an aggregate unless calculations cite every operand. Remove numeric source identifiers that are not themselves evidenced. " +
            ...
            "Preserve units, periods, baselines, missing values and qualifications. Never invent facts, commitments, quotes or causal claims. When repairing an existing deck, treat its chart blanks or stale values as defects to verify against the source the user designated as authoritative; do not copy a blank current-period point solely because the old slide omitted it. If the authoritative source is genuinely incomplete, disclose that gap instead of inventing a value. A repaired slide is final audience copy: never leave 'recalc pending', 'stale - recalc', or other work-in-progress placeholders visible. " +
            ...
            "A requested headline results slide is a content slide, not a cover or divider. Use a scorecard when it reports multiple sourced metrics; create a cover only when the user explicitly asks for one. " +
        ...
        public const string FactReview =
            ...
            "When the user names an attached workbook as the authority for current-period facts, a blank or stale value in the old presentation is a repair target, not evidence that the workbook period must remain blank. Check the workbook receipts and any host-verified calculations before demanding a missing-value disclosure. Do not invent a point if those records are genuinely incomplete. " +
            ...
            "When the user explicitly restricts a chart to primary values only, do not demand a secondary cost, budget or other series even if a model-written brief mentions both measures. Check any secondary comparison claim against its cited evidence in text or a separate table; the host independently enforces the one-series chart constraint. Never turn a conflicting brief into a new user requirement. " +
        ...
        public const string DeckReview =
            ...
            "terminology, periods, units, visual consistency and whether the business question is answered. Judge it as an executive presentation at thumbnail size, not merely as readable Office geometry. Reject plain body-text dumps, genuinely accidental whitespace, weak focal hierarchy, exact redundant conclusions and monotonous composition. A takeaway may reprise a topic when it adds a distinct sourced fact; whitespace framing a substantial chart or table is intentional. Slide numbering is host-owned from native slide order, not the logical batch ordinal. Evidence may be dense, but density is never permission to paste a report onto a slide. " +
        ...
        public static void ValidateRepairCompleteness(string prompt, IEnumerable<IDictionary<string, object>> slides)
        {
            if (!Regex.IsMatch(prompt ?? "", @"(?i)\b(?:repair|repaired|fix\s+stale)\b")) return;
            foreach (var slide in slides ?? Enumerable.Empty<IDictionary<string, object>>())
                if (VisibleStrings(slide).Any(text => Regex.IsMatch(text ?? "",
                    @"(?i)\b(?:recalc(?:ulation|ulate)?\s+pending|stale\s*[-–—:]\s*recalc|correction\s+pending)\b")))
                    throw new InvalidOperationException("SLIDE_REPAIR_PLACEHOLDER: A repaired slide still displays a stale or recalculation-pending placeholder. Use the designated authoritative source to replace it with verified final copy, or disclose a genuinely unavailable fact without claiming the repair is done.");
        }
