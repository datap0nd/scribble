...
namespace Scribble.Office
{
    ...
    public static class SamsungAuthoringPolicy
    {
        ...
        public const string Instructions =
            ...
            "If mandatory content cannot fit the exact count, explain the conflict and ask which constraint may change before the first slide is written. " +
            "Once a slide has been written, never call ask_user: the request, audience, period, units and format are settled, so resolve derived values through calculations, omit optional facts the source does not state, finish every remaining planned ID, and state any unmet requirement in the final response instead. " +
