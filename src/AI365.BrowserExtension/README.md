# AI365 browser extension

This is one Manifest V3 extension for both Microsoft Edge and Google Chrome. It opens AI365 in the browser side panel and sends chat requests to the local native-messaging host named `com.ai365.browser`.
...
Install AI365 browser support first. AI365 Setup must install and register the native bridge, and its native-host manifest must allow this extension origin:
...
5. Select this `AI365.BrowserExtension` folder—the folder that directly contains `manifest.json`.
6. Pin AI365 from Edge's Extensions menu if you want the toolbar button to remain visible.
7. Click the AI365 toolbar button to open the side panel.
...
4. Choose **Load unpacked**.
5. Select this `AI365.BrowserExtension` folder—the folder that directly contains `manifest.json`.
6. Pin AI365 from Chrome's Extensions menu if you want the toolbar button to remain visible.
7. Click the AI365 toolbar button to open the side panel.

After replacing extension files with a newer version, return to the browser's Extensions page and choose **Reload** on AI365.
...
open AI365 Settings in an Office app, enter that tool's exact MCP name in the
...
- `activeTab`: temporary access to the tab where the user invoked AI365; there is no permanent access to every website.
- `scripting`: reads selection or page text only after an attach button or AI365 context-menu action.
- `contextMenus`: provides “Ask AI365 about this page” and “Ask AI365 about this selection.”
- `nativeMessaging`: talks to the locally installed `com.ai365.browser` bridge.
- `sidePanel`: hosts the AI365 conversation beside the current page.

There are no host permissions, no `<all_urls>` access, no content scripts, no remote scripts, and no background page collection.

## Troubleshooting

- **Browser support is not installed:** rerun AI365 Setup with browser support enabled, then restart the browser.
- **The extension is not authorized:** the extension and AI365 bridge are from different builds, or the native-host manifest does not contain the stable origin above. Reinstall matching versions.
- **No model is configured:** open AI365 Settings from an Office app and select/configure a model.
...
- **A new tab cannot be read while the side panel stays open:** click the AI365 toolbar button on that tab to grant temporary `activeTab` access, then attach it.
