...
namespace GuardrailTests
{
    internal static class OfficeBootstrapTests
    {
        ...
        public static void EncodedSlideWithOneTransposedTableRowsBracketKeepsContent()
        {
            ...
            var slide = new Dictionary<string, object> {
                ...
                { "claims", new[] { new Dictionary<string, object> {
                    { "text", "South revenue 22,675 EUR" }, { "evidence", "South 22675 10787" },
                    { "label", "RevenueEUR" }, { "unit", "EUR" }, { "period", "2026-06" }
                } } }
            };
            var encoded = json.Serialize(new[] { slide });
            var malformed = encoded.Replace("\"10,787\"]]}", "\"10,787\"]}]");
            Check(malformed != encoded, "The regression fixture did not transpose the rows bracket.");
            ...
            Check(ToolContractValidator.Validate(call, PresentationToolCatalog.DraftDefinition()).Count == 0,
                "A transposed table rows bracket stalled a complete encoded slide.");
