...
function newSlide(deck,spec,title,n){
  const s=deck.slides.add();s.background.fill='#FFFFFF';
  textbox(s,title,78,38,1152,72,32,true,INK);
  textbox(s,`${spec.company} | ${spec.domain} | January–June 2026`,49,116,1181,31,18,false,BLUE);
...
function buildDeck(spec,defective){
  ...
  let s=newSlide(deck,spec,`${spec.company}: ${spec.domain} review`,1);
  ...
  textbox(s,`${spec.secondary_label}\n${display(spec.current.secondary)}`,735,220,480,144,30,false,INK);
  ...
  s=newSlide(deck,spec,`${spec.primary_label}: six-month trend`,2);nativeChart(s,spec,{defective});
  s=newSlide(deck,spec,'June results by operating group',3);
  ...
  s=newSlide(deck,spec,'Operating review and evidence boundaries',4);
  ...
  textbox(s,commentary,84,204,defective?330:1110,defective?84:390,24,false,INK);
  s=newSlide(deck,spec,'Follow-up register',5);
  ...
  s=newSlide(deck,spec,'Source coverage and definitions',6);
